...
$users = [
    ...
    ['id' => 4, 'username' => 'user3', 'password' => 'password3', 'email' => 'user3@example.com'],
];
...
function query($sql) {
    ...
    if (strpos($sql, 'select') === 0) {
        // 检查是否包含注入
        if (strpos($sql, 'union') !== false || strpos($sql, 'or') !== false || strpos($sql, 'and') !== false) {
            return [
                ['id' => 999, 'username' => 'FLAG{SQL_INJECTION_SUCCESS}', 'password' => 'SQL_INJECTION', 'email' => 'flag@example.com'],
                ['id' => 1000, 'username' => '数据库信息', 'password' => 'version: 5.7.31', 'email' => 'database: testdb']
            ];
        }
