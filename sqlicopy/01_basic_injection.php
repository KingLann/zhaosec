...
$stmt->execute([$id]);<br>
        ...
        <div class="card">
            <h3>⚠️ SQL注入的危害</h3>
            <div class="info-section">
                <strong>数据层面危害：</strong>数据泄露、数据篡改、数据删除、拖库<br>
                <strong>系统层面危害：</strong>权限绕过、服务器控制、内网渗透、植入后门
            </div>
        </div>
...
SELECT id, username FROM users WHERE id > 5;<br>
<br>
-- 2. 插入数据（INSERT）<br>
...
<br>
-- 3. 查询表的列信息<br>
SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'users';<br>
-- 4. 查询MySQL版本<br>
SELECT version();<br>
<br>
-- 5. 查询用户权限<br>
SELECT user, host FROM mysql.user;<br>
        ...
        <div class="card">
            <h3>🛡️ 防御方法</h3>
            <div class="info-section">
                <strong>核心原则：</strong>永远不要信任用户输入，始终对输入数据进行验证和净化<br>
                <strong>推荐防御：</strong>参数化查询、存储过程、ORM框架、输入验证、最小权限<br>
                <strong>辅助防御：</strong>WAF防护、错误处理、日志监控、代码审计、安全培训
            </div>
...
$result = $stmt->get_result();<br>
            ...
            <div class="alert alert-info mt-3">
                <strong>💡 提示：</strong>本靶场仅供学习SQL注入原理和防御方法，请勿用于非法用途。
            </div>
        </div>
    </div>
